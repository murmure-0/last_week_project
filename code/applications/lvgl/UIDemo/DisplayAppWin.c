 ...
 * 根据页面ID获取对应的窗口对象
 * 
 * @param pageID 页面ID，用于标识不同的页面
 * @return 返回对应页面ID的窗口对象指针，如果页面ID超出范围，则返回NULL
 *
 * 此函数通过页面ID从一个全局数组中检索并返回相应的窗口对象指针。
 * 页面ID必须在有效范围内，即小于定义的最大页面数PAGE_MAX。
 * 如果页面ID超出范围，函数将返回NULL，表示未找到对应的窗口对象。
 */
