...
//  * 运行所有的测试用例。
//  * 
//  * 该函数遍历一个测试用例数组，每个用例都会被调用并检查其回调函数的返回值以确定是否通过。
//  * 如果所有测试用例都通过，将会改变相关按钮的颜色以提示测试结果。
//  */
// void run_tests(void)
// {
//     /* 初始化通过的测试用例计数器 */
...
//  * @brief 初始化测试界面
//  * 
//  * 本函数负责将应用程序窗口移至前台，并创建和配置测试界面的各个组件，
//  * 包括内容区域、标题和列表。此外，它还设置了一个页面动画并启动测试。
//  * 这个函数在程序启动或需要重新初始化测试界面时调用。
//  */
// static void Setup()
// {
//     /* 将应用程序窗口移至前台，使其成为活动窗口 */
//     /*将此页面移到前台*/
